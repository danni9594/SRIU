...
train_config = {'method': 'full_retraining',
                'dir_name': 'tmall_MF_FR_10.19-10.30',  # edit based on dataset, model, method, period range
...
for lr in [1e-2]:
    ...
    for period in range(train_config['start_period'], train_config['end_period'] + 1, train_config['period_length']):
        ...
        train_config['train_start_period'] = 20141001
        ...
        train_config['model_alias'] = 'MF_FR_' + datetime.strptime(str(period), '%Y%m%d').strftime('%m.%d')
        ...
        if period == train_config['start_period']:
            ...
            prev_model_alias = 'MF_FR_' + datetime.strptime(str(period - train_config['period_length']), '%Y%m%d').strftime('%m.%d')
