...
class SampleGenerator:
    ...
    def current_set(self):
        current_set = self.data_df[(self.data_df['period'] >= self.config['train_start_period'])
                                   & (self.data_df['period'] <= self.config['train_end_period'])]
        current_set = current_set.drop(['period'], axis=1)
        return current_set

    def pos_current_set(self):
        pos_current_set = self.data_df[(self.data_df['period'] >= self.config['train_start_period'])
                                       & (self.data_df['period'] <= self.config['train_end_period'])
                                       & (self.data_df['label'] == 1)]
        return pos_current_set

    def init_reservoir(self):
        pos_pretrained_set = self.data_df[(self.data_df['period'] < self.config['start_period'])
                                          & (self.data_df['label'] == 1)]
        pos_history_D_size = len(pos_pretrained_set)
        reservoir_size = int(pos_history_D_size * self.config['frac_of_history_D'])
        reservoir = pos_pretrained_set.sample(n=reservoir_size)
        return reservoir
